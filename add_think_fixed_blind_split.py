...
def train(answer_model: GPT2SplitModel, think_model: GPT2SplitModel, cfg: TrainingConfig, dataset: pd.DataFrame):
    ...
    wandb.init(project="add_thoughtful_think", name=f"think_fixed_blind_split{input_max}", config=cfg)
    ...
    for b in (tr:=tqdm.trange(len(dataset), ncols=160)):
        row = dataset.iloc[b]
        ...
        if b != 0 and b % cfg.batch_size == 0:
            ...
            tr.set_description(f"{magenta}pred reward mean: {pred_reward_mean:.3f}, think reward: {think_reward:.3f}, epsilon: {epsilon:.3f}, bench acc: {benchmark_accuracy:.3f}")
